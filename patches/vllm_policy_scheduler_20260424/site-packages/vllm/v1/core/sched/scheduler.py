...
from vllm.v1.core.sched.policy import get_scheduler_policy_plugin
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.policy_plugin = get_scheduler_policy_plugin(self.policy.value)
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                request_queue, request = self._select_waiting_request_for_scheduling()
    ...
    def _select_waiting_queue_for_scheduling(self) -> RequestQueue | None:
        ...
        # Policy heap modes: compare queue heads when both queues are non-empty.
        if self.waiting and self.skipped_waiting:
            waiting_req = self.waiting.peek_request()
            skipped_req = self.skipped_waiting.peek_request()
            waiting_key = self.policy_plugin.sort_key(waiting_req)
            skipped_key = self.policy_plugin.sort_key(skipped_req)
    ...
    def _select_waiting_request_for_scheduling(
        self,
    ) -> tuple[RequestQueue | None, Request | None]:
        if self.policy == SchedulingPolicy.MOE_AFFINITY and hasattr(
            self.policy_plugin, "dynamic_sort_key"
        ):
            topk = getattr(self.policy_plugin, "rerank_topk", 8)
            candidates: list[tuple[tuple[object, ...], RequestQueue, Request]] = []
            for request_queue in (self.waiting, self.skipped_waiting):
                if not request_queue:
                    continue
                for request in itertools.islice(request_queue, topk):
                    key = self.policy_plugin.dynamic_sort_key(request, self.running)
                    candidates.append((key, request_queue, request))

            if candidates:
                _, request_queue, request = min(candidates, key=lambda item: item[0])
