...
SchedulerPolicy = Literal["fcfs", "priority", "length_only", "ltr", "moe_affinity"]
...
class SchedulerConfig:
    ...
    - "length_only" means waiting requests are handled by ascending estimated
      total length (`prompt_tokens + max_tokens`), with arrival order used as a
      tie breaker.
    - "ltr" means waiting requests are handled by ascending external
      learning-to-rank score from `VLLM_LTR_SCORE_FILE`, with length and
      arrival order used as tie breakers.
    - "moe_affinity" means waiting requests keep a length/LTR-like base order,
      then a small top-K window is reranked by expert-signature affinity to the
